...
.autocomplete_pane.right_pane input {
  ...
  font-size: 14px !important;
  font-family: -apple-system, BlinkMacSystemFont, "Malgun Gothic", "맑은 고딕", Helvetica, Arial, sans-serif !important;
  transition: all 0.2s cubic-bezier(0.2, 1, 0.5, 0.95) !important;
}

.autocomplete_pane.right_pane input:hover {
  background: rgb(33, 34, 37) !important;
...
.autocomplete_pane.right_pane div {
  color: rgb(255, 255, 255) !important;
  font-size: 14px !important;
  font-family: -apple-system, BlinkMacSystemFont, "Malgun Gothic", "맑은 고딕", Helvetica, Arial, sans-serif !important;
...
const applyRightPaneStyles = () => {
  ...
  // Div 라벨 스타일 적용
  const divs = rightPane.querySelectorAll('div');
  divs.forEach(div => {
  ...
  // Button 요소 스타일 적용
  const buttons = rightPane.querySelectorAll('button');
  buttons.forEach(button => {
    ...
    button.style.transition = 'all 0.2s cubic-bezier(0.2, 1, 0.5, 0.95)';
  });
};

// Right Pane 요소가 생성/변경될 때마다 적용
const rightPaneObserver = new MutationObserver(() => {
  applyRightPaneStyles();
});

// Right Pane 감시 시작
const startRightPaneObserver = () => {
  const rightPane = document.querySelector('.autocomplete_pane.right_pane');
  if (rightPane) {
    rightPaneObserver.observe(rightPane, { 
      childList: true, 
      subtree: true, 
      attributes: true, 
